...
**Must Do:** Use {{auth_strategy}} for session/token management; Hash passwords before storage; Return consistent error shapes from all auth endpoints
**Cannot Do:** Store plaintext passwords; Skip input validation on any auth endpoint
**Should Prefer:** Reuse existing middleware patterns in the codebase; Keep auth logic in a dedicated module (not scattered across routes)
**Should Escalate:** If the database schema needs migration — pause and confirm before running

## Decomposition

### Sub-task 1: User model and password hashing
...
### Sub-task 2: Registration and login endpoints
- **Input:** User model from Sub-task 1
...
### Sub-task 3: Session middleware
- **Input:** {{auth_strategy}} library, login endpoint from Sub-task 2
...
### Sub-task 4: Logout and session invalidation
- **Input:** session middleware from Sub-task 3
- **Output:** POST /auth/logout that invalidates token
- **Success criteria:** token rejected after logout

### Sub-task 5: Integration tests
- **Input:** all auth endpoints from Sub-tasks 1-4
...
|-----------|-------|-----------------|
| Happy path register → login → access → logout | fresh database, valid credentials | 201 register, 200 login with token, 200 on protected route, 200 logout, 401 after logout |
| Invalid credentials | wrong password | 401 with no information leakage |
| Missing token | protected route with no Authorization header | 401 |
