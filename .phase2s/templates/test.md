...
**Must Do:** Test edge cases, not just happy paths; Each test must have a clear, readable name
**Cannot Do:** Modify production code to make tests pass; Write tests that test implementation details (not behavior)
**Should Prefer:** Group related tests in describe() blocks; Use table-driven / parameterized tests for multiple similar cases
**Should Escalate:** If production code has a bug discovered during testing — file a separate fix, don't patch here

## Decomposition

### Sub-task 1: Coverage audit
...
### Sub-task 2: Happy path tests
- **Input:** audit from Sub-task 1
- **Output:** tests for the main success scenarios
- **Success criteria:** {{test_command}} passes; coverage improves

### Sub-task 3: Error and edge case tests
- **Input:** audit from Sub-task 1, happy path tests from Sub-task 2
...
|-----------|-------|-----------------|
| Coverage gate | run test suite with coverage | {{module_name}} coverage ≥ {{coverage_target}}% |
