...
- **Navigation sidebar** — 220px desktop sidebar with Runs, Live, Config, and Help items. Live/Config/Help are marked "Coming in Sprint 95" at reduced opacity. Collapses to 48px icon-only on tablet.
...
- **`phase2s conduct-log` CLI command** — Read and display the run history table (Date | Goal | Subtasks | Result | Duration). Flags: `--last` (most recent entry only), `--limit N` (newest N entries, default 10), `--json` (raw JSONL to stdout). CLI-only; not yet an MCP tool.
