...
- [x] REPL UX polish — `:dump` and `:dump html` export the current session as a markdown transcript or self-contained HTML page (dark mode, monospace, word-wrap) written to `.phase2s/exports/`. `:help` prints all REPL commands with descriptions so you never need to look up the cheatsheet. Fuzzy `@file` Tab completion upgraded: Tab on `@agt` now recursively walks the project tree and returns all filenames whose basename contains "agt" — no need to type the full path prefix. Path traversal hardened: `../` fragments blocked at both the `resolve()` and `relative()` level; dotfiles excluded from completions. All three features reviewed and hardened before ship. 30 new tests. 1,660 tests. (v1.41.0)
